...
class MyBorderAnimation extends StatefulWidget {
  @override
  _MyBorderAnimationState createState() => _MyBorderAnimationState();
}

class _MyBorderAnimationState extends State<MyBorderAnimation> {
  int value = 0;
  Timer timer;
  @override
  void initState() {
    ...
    return Container(
      child: SquarePercentIndicator(
        ...
        child: Center(
          ...
          child: Text(
            " 0000000000000000bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb%",
...
class RadialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.arcTo(Rect.fromCircle(center: Offset(size.width - borderRadius, borderRadius), radius: borderRadius), -pi / 2, pi / 2, false);
    path.lineTo(size.width, size.height - borderRadius);
    path.arcTo(Rect.fromCircle(center: Offset(size.width - borderRadius, size.height - borderRadius), radius: borderRadius), 0, pi / 2, false);
    path.lineTo(0 + borderRadius, size.height);
    path.arcTo(Rect.fromCircle(center: Offset(borderRadius, size.height - borderRadius), radius: borderRadius), pi / 2, pi / 2, false);
    ...
    if (reverse) {
      dashPath = dashPath.transform(Matrix4Transform().rotate(pi / 2).m.storage).transform(Matrix4Transform().flipHorizontally().m.storage);
    }
...
class Matrix4Transform {
  ...
  /// If you define an origin it will have that point as the axis of rotation.
  Matrix4Transform rotateDegrees(double angleDegrees, {Offset origin}) => rotate(_toRadians(angleDegrees), origin: origin);

  /// Rotates by [angleDegrees] degrees (0 to 360 one turn), clockwise.
  /// The axis of rotation will be the center of the object with the given size.
  Matrix4Transform rotateByCenterDegrees(double angleDegrees, Size size) => rotateByCenter(_toRadians(angleDegrees), size);
  ...
  Matrix4Transform direction(double directionRadians, double distance) => translateOffset(Offset.fromDirection(directionRadians, distance));
...
class Matrix4TransformTween extends Tween<Matrix4Transform> {
  ...
  /// filled in later.
  Matrix4TransformTween({@required Matrix4Transform begin, @required Matrix4Transform end}) : super(begin: begin, end: end);
